...
def start_gen():
    ...
    try:
        ...
        return jsonify({"status": "success", "taskId": res['data']['createGenerationTask']['id']})
    except: return jsonify({"status": "error"})

# --- STEP B: CHECK STATUS (New Logic) ---
@app.route('/api/check_status', methods=['POST'])
def check_status():
    d = request.json
    token, tid = d.get("token"), d.get("taskId")
    try:
        ...
        if task['status'] == "completed":
            return jsonify({"status": "completed", "images": [i['url'] for i in task['media']['urls'] if i['variant'] == "PUBLIC"]})
        return jsonify({"status": task['status']})
    except: return jsonify({"status": "error"})

# --- HISTORY, SEARCH, CREDITS, CLAIMS (UNCHANGED) ---
