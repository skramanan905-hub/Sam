...
def generate():
    ...
    helper = d.get("helper", True) # NEW: Helper Toggle Logic
    ...
    payload = {
        ...
        "variables": {
            "parameters": {
                ...
                "promptHelper": {"withStage": helper, "userWantToEnable": helper, "enable": helper} # Modified with variable
            },
    ...
    try:
        ...
        for edge in reversed(edges):
            ...
            if node.get('status') == "completed":
                ...
                # NEW: Extract both prompts
                original = node.get('extra', {}).get('naturalPrompts', [""])[0]
                final = node.get('parameters', {}).get('prompts', "")
                if img: 
                    task_data.append({
                        "url": img, 
                        "prompt_original": clean_txt(original), 
                        "prompt_final": clean_txt(final)
                    })
...
# (Search, Credits, Claim, Upload remain 100% same as previous code)
