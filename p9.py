...
# --- UPDATED: NEW TO OLD (LATEST FIRST) HISTORY ---
@app.route('/api/tasks', methods=['POST'])
def get_web_tasks():
    d = request.json
    t, cursor = d.get("token"), d.get("cursor")
    # Using 'last' and 'before' ensures we get the most recent ones first
    p = {
        "operationName": "listMyTasks",
        "variables": json.dumps({"last": 30, "before": cursor, "parameterFields": ["extra", "prompts"]}),
    ...
    try:
        ...
        # Reverse the edges so Newest is at the top of the grid
        edges = r['data']['me']['tasks']['edges'][::-1]
        ...
        return jsonify({
            "status": "success", 
            "tasks": task_data, 
            "cursor": page_info['startCursor'] if page_info['hasPreviousPage'] else None
        })
