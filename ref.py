...
# Hashes from your Reqable logs (V6)
...
def gen_step4(m):
    ...
    if user_states[uid]['mode'] == "Reference Image":
        bot.send_message(uid, "STEP 4: Now send the Photo for reference:")
...
def execute_gen(uid):
    ...
    bot.send_message(uid, "Accessing PixAI Engine...", reply_markup=menu())
    ...
    except: bot.send_message(uid, "Generation Error.")
...
def fetch_all(m):
    ...
    r = requests.get(API_URL, params={"operationName": "listMyTasks", "variables": json.dumps(v), "extensions": json.dumps({"persistedQuery": {"version": 1, "sha256Hash": H_LIST}})}, headers=get_h(t)).json()
