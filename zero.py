...
# All Hashes memorized from your Reqable logs
    ...
    payload = {
        ...
        "variables": {
            "parameters": {
                ...
                "modelId": d.get("model_id", "1861558740588989558"), 
    ...
    try:
        ...
        for edge in edges:
            ...
            if node['status'] == "completed":
                ...
                task_data.append({
                    "id": node['id'], "url": node['media']['urls'][0]['url'],
                    ...
                    "cfg": params.get('cfgScale', 5), "size": f"{params.get('width')}x{params.get('height')}",
                    "model": node.get('modelId', "Haruka v2"), "loras": params.get('loraParameters', [])
                })
...
def search():
    d = request.json
    h = H_SEARCH_MODEL if d.get('is_model') else H_SEARCH_LORA
    t = "ANY_MODEL" if d.get('is_model') else "ANY_LORA"
    v = {"keyword": d.get("keyword"), "feed": "meilisearch", "types": [t], "first": 20, "after": d.get("cursor")}
    r = requests.get(API_URL, params={"operationName":"listGenerationModels","variables":json.dumps(v),"extensions":json.dumps({"persistedQuery":{"version":1,"sha256Hash":h}})}, headers=get_h(d.get("token"))).json()
