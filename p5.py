...
# ================= CONFIGURATION (EXACT MATCH FROM YOUR BOT) =================
...
# --- 1. TASK LIST (THE MAIN FOCUS) ---
@app.route('/api/tasks', methods=['POST'])
def get_web_tasks():
    t = request.json.get("token")
    # Matches your bot's exact list variables and HASH
    p = {
        "operationName": "listMyTasks",
        "variables": json.dumps({"last": 30, "parameterFields": ["extra", "prompts"]}),
    ...
    try:
        ...
        for edge in edges:
            ...
            if node.get('status') == "completed":
                # Extract image URL and Prompt logic
                img_url = node.get('media', {}).get('urls', [{}])[0].get('url')
                prompt_text = node.get('parameters', {}).get('prompts', "")
                
                if img_url:
                    task_data.append({
                        "url": img_url,
                        "prompt": clean_txt(prompt_text)
                    })
        return jsonify({"status": "success", "tasks": task_data})
    except:
        return jsonify({"status": "error"})

# --- 2. GENERATION (EXACT 15s POLLING LOGIC) ---
    ...
    payload = {
        ...
        "variables": {
            "parameters": {
                ...
                "lora": weights, "loraParameters": params, "mediaId": mid, "strength": strength, 
...
# --- 3. REFERENCE UPLOAD (3-STEP LOGIC) ---
