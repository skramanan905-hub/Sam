...
def run_generation():
    ...
    batch, mid = int(d.get("batch", 1)), d.get("mid")
    ...
    neg = d.get("neg", "")
    user_triggers = d.get("triggers", "").strip()
    ...
    for lid in lora_ids:
        ...
        if meta:
            ...
            # Store the default trigger in case user input is empty
            fetched_triggers += f"{meta['trigger']}, "
            
            # Logic: If user provided text, use it. Otherwise use API default.
            active_trigger = user_triggers if user_triggers else meta['trigger']
            l_p.append({
                "versionId": meta['v_id'], 
                "weight": lora_str, 
                "triggerWords": active_trigger, 
    ...
    payload = {
        ...
        "variables": {
            "parameters": {
                ...
                "lora": l_w, "loraParameters": l_p, "mediaId": mid, "strength": strength,
...
# --- HISTORY (REVERSED: NEWEST FIRST), SEARCH (SMALL GRID), CREDITS, CLAIM ---
