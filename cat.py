...
# Literal query to prevent PersistedQueryNotFound errors
Q_OLD_REW = "mutation followSocialMedia($platform: String!) { followSocialMedia(platform: $platform) { success __typename } }"
...
def search_models():
    d = request.json
    v = {"keyword": d.get("keyword"), "feed": "preset" if d.get("sort")=="trending" else "meilisearch", "sort": d.get("sort", "most_used"), "types": ["ANY_MODEL"], "first": 30, "after": d.get("cursor")}
    # --- EXACT CAT.PY SEARCH MAPPING ---
...
def search_loras():
    d = request.json
    v = {"keyword": d.get("keyword"), "feed": "meilisearch", "sort": d.get("sort", "most_used"), "types": ["ANY_LORA"], "first": 30, "after": d.get("cursor")}
...
def claim_old():
    ...
    for p in ["tiktok", "youtube", "instagram", "twitter", "discord"]:
        # FIXED: Added Q_OLD_REW to prevent PersistedQueryNotFound
        r = requests.post(API_URL, json={"operationName":"followSocialMedia","variables":{"platform":p}, "query": Q_OLD_REW, "extensions":{"persistedQuery":{"version":1,"sha256Hash":H_REW}}}, headers=h)
