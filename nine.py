...
# ALL HASHES MEMORIZED FROM YOUR LOGS
...
# --- UPDATE 1: FULL HISTORY (MATCHES YOUR SS) ---
...
def tasks():
    ...
    for e in edges:
        ...
        if n['status'] == "completed":
            params = n['parameters']
            t_data.append({
                "id": n['id'], "url": n['media']['urls'][0]['url'], "prompt": params.get('prompts'), "neg": params.get('negativePrompts'),
                "steps": params.get('samplingSteps'), "method": params.get('samplingMethod'), "cfg": params.get('cfgScale'),
                "size": f"{params.get('width')}x{params.get('height')}", "model": n.get('modelId', "Haruka v2"),
                "loras": params.get('loraParameters', [])
            })
    return jsonify({"tasks": t_data, "cursor": r['data']['me']['tasks']['pageInfo']['startCursor']})
...
# --- UPGRADED GENERATION (PRIORITY + POLICY BYPASS) ---
...
def generate():
    ...
    # Priority Logic (Update requested)
    priority = 1000 if d.get('high_priority') else 0

    p = {"operationName": "createGenerationTask", "variables": {"parameters": {"prompts": d['prompt'] + trigs, "negativePrompts": d['neg'], "modelId": d.get('model_id', "1861558740588989558"), "width": int(d['w']), "height": int(d['h']), "batchSize": int(d['batch']), "lora": l_w, "loraParameters": l_p, "samplingSteps": int(d['steps']), "samplingMethod": "Euler a", "cfgScale": float(d['cfg']), "priority": priority, "promptHelper": {"withStage": True, "userWantToEnable": True, "enable": True}}, "extra": {"naturalPrompts": [d['prompt']]}}, "extensions": {"persistedQuery": {"version": 1, "sha256Hash": H_GEN}}}
    
    r = requests.post(API_URL, json=p, headers=get_h(d['token'])).json()
