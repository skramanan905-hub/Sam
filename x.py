...
def main_menu():
    ...
    m.row("Login / Update Token", "Refresh Bot") # Added Refresh Button
    return m
...
def refresh(m):
    ...
    if uid in user_search_history: del user_search_history[uid]
    if uid in id_memory: del id_memory[uid]
    bot.send_message(uid, "♻️ **System Refreshed.** Memory cleared.", reply_markup=main_menu(), parse_mode="Markdown")
...
def balance(m):
    ...
    except: bot.send_message(uid, "❌ Error syncing balance.")
...
def callbacks(call):
    ...
    if call.data.startswith("selid_"):
        ...
        if uid in id_memory and idx < len(id_memory[uid]):
            name = clean_txt(id_memory[uid][idx]['name'])
            lora_id = id_memory[uid][idx]['id']
            bot.send_message(uid, f"{name} : `{lora_id}`", parse_mode="Markdown")
