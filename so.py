...
H_COST   = "b258287431e21b068e5473e04400e99887754d92497645f5673d676cc8ec1797" # Cost Hash
...
def tasks():
    ...
    try:
        ...
        rj = r.json()
        edges = rj['data']['me']['tasks']['edges']
        task_data = []
        for edge in reversed(edges):
            node = edge['node']
            if node['status'] == "completed":
                p_node = node['parameters']; extra = p_node.get('extra', {})
                
                # --- EXACT PROMPT LOGIC PRESERVED ---
                ...
                else: orig_prompt = "Not Found"

                ref_id = p_node.get('mediaId')
                task_data.append({
                    "url": node['media']['urls'][0]['url'],
                    "p_orig": clean_txt(orig_prompt), "p_final": clean_txt(p_node.get('prompts')),
                    "neg": clean_txt(p_node.get('negativePrompts', "")), "id": node['id'],
                    "time": format_pixai_time(node.get('createdAt')),
                    "size": f"{p_node.get('width')}x{p_node.get('height')}",
                    "steps": p_node.get('samplingSteps'), "cfg": p_node.get('cfgScale'), "method": p_node.get('samplingMethod'),
        ...
        return jsonify({"status": "success", "tasks": task_data, "cursor": rj['data']['me']['tasks']['pageInfo']['startCursor'] if rj['data']['me']['tasks']['pageInfo']['hasPreviousPage'] else None, "refreshed_token": check_refresh(r)})
    except: return jsonify({"status": "error"})
...
def compute_cost():
    ...
    vars = {"parameters": {"modelId": d.get("modelId"), "width": int(d.get("w")), "height": int(d.get("h")), "batchSize": int(d.get("batch")), "lora": l_w, "samplingSteps": int(d.get("steps"))}}
    p = {"operationName": "computeTaskCost", "variables": json.dumps(vars), "extensions": json.dumps({"persistedQuery": {"version": 1, "sha256Hash": H_COST}})}
    r = requests.get(API_URL, params=p, headers=get_h(d.get("token")))
    return jsonify({"cost": r.json()['data']['computeTaskCost']['totalCost'], "refreshed_token": check_refresh(r)})
...
def generate():
    ...
    payload = {"operationName": "createGenerationTask", "variables": {"parameters": {"prompts": prompt + ", " + all_t, "negativePrompts": d.get("neg"), "modelId": modelId, "width": int(d.get("w")), "height": int(d.get("h")), "batchSize": int(d.get("batch")), "lora": l_w, "loraParameters": l_p, "mediaId": d.get("mediaId"), "strength": float(d.get("strength")), "samplingSteps": int(d.get("steps")), "samplingMethod": "Euler a", "cfgScale": float(d.get("cfg")), "promptHelper": {"withStage": True, "userWantToEnable": True, "enable": True}}, "extra": {"naturalPrompts": [prompt]}}, "extensions": {"persistedQuery": {"version": 1, "sha256Hash": H_GEN}}}
    
    try:
        r_init = requests.post(API_URL, json=payload, headers=get_h(token))
        tid = r_init.json()['data']['createGenerationTask']['id']
        while True: # --- EXACT POLLING LOGIC ---
            time.sleep(15)
...
def search_models():
    d = request.json
    v = {"keyword": d.get("keyword"), "sort": d.get("sort", "MOST_USED"), "feed": "meilisearch", "types": ["ANY_MODEL"], "first": 30, "after": d.get("cursor")}
    ...
    for e in res['data']['generationModels']['edges']:
        n = e['node']; v_id = n['latestAvailableVersion']['id'] if n.get('latestAvailableVersion') else n['id']
        items.append({"name": n['title'], "id": v_id, "type": n.get('modelType', 'Model'), "usage": f"{n.get('usageCount',0):,}", "likes": n.get('likeCount',0), "thumb": next((u['url'] for u in n['media']['urls'] if u['variant'] == "STILL_THUMBNAIL"), "")})
...
def search_loras():
    d = request.json
    v = {"keyword": d.get("keyword"), "sort": d.get("sort", "MOST_USED"), "feed": "meilisearch", "types": ["ANY_LORA"], "first": 30, "after": d.get("cursor")}
    ...
    for e in res['data']['generationModels']['edges']:
        n = e['node']
        items.append({"name": n['title'], "id": n['id'], "usage": f"{n.get('usageCount',0):,}", "likes": n.get('likeCount',0), "thumb": next((u['url'] for u in n['media']['urls'] if u['variant'] == "STILL_THUMBNAIL"), "")})
    return jsonify({"results": items, "cursor": res['data']['generationModels']['pageInfo']['endCursor'] if res['data']['generationModels']['pageInfo']['hasNextPage'] else None, "refreshed_token": check_refresh(r)})
...
def lora_meta():
    ...
    r = requests.get(API_URL, params=p, headers=get_h(d.get("token")))
    res = r.json(); v = res['data']['generationModel']['latestAvailableVersion']
    return jsonify({"v_id": v['id'], "trigger": v['extra'].get('triggerWords', ""), "name": res['data']['generationModel']['title'], "id": d.get("id"), "refreshed_token": check_refresh(r)})
