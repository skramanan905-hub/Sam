...
# --- DEEP HISTORY LOGIC (REPLACED ONLY THIS PART) ---
@app.route('/api/tasks', methods=['POST'])
def tasks():
    d = request.json
    t, cursor = d.get("token"), d.get("cursor")
    # All required detail fields from your specific snippet
    ...
    try:
        ...
        task_data = []
        for edge in reversed(edges): # Newest on Top
            node = edge['node']
            if node['status'] == "completed":
                params = node['parameters']
                extra = params.get('extra', {})
                
                # --- DEEP PROMPT FALLBACK LOGIC ---
                orig_prompt = "N/A"
                if extra.get('naturalPrompts') and isinstance(extra['naturalPrompts'], list):
                    orig_prompt = extra['naturalPrompts'][0]
                elif extra.get('naturalPrompt'):
                    orig_prompt = extra['naturalPrompt']
                else:
                    orig_prompt = params.get('prompts', 'N/A')

                # LoRA Parameters List
                loras_applied = [{"trigger": l.get('triggerWords'), "weight": l.get('weight')} for l in params.get('loraParameters', [])]

                # Reference Image URL logic
                ref_id = params.get('mediaId')
                ref_url = f"https://images-ng.pixai.art/images/orig/{ref_id}" if ref_id else None
...
# (Existing search, generate, credits logic remains identical below)
