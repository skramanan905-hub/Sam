...
def generate():
    ...
    try:
        ...
        # REVERTED TO NORMAL POLLING (No streaming)
        for _ in range(10): # 10 times x 50s = 8 minutes max
            time.sleep(50) # CHANGED FROM 15 TO 50
    ...
    try:
        ...
        for edge in reversed(edges):
            ...
            if node.get('status') == "completed":
                ...
                # FIXED HISTORY LOGIC: Take full string, not index [0] of a string
                raw_natural = extra.get('naturalPrompts', "")
                if isinstance(raw_natural, list) and len(raw_natural) > 0:
                    original = raw_natural[0]
                else:
                    original = raw_natural # Use whole string if it's not a list

                final = params.get('prompts', "")
                task_data.append({
                    "url": img, "p_orig": clean_txt(original if original else final), "p_final": clean_txt(final),
                    "neg": params.get('negativePrompts', ""), "steps": params.get('samplingSteps', ""),
                    "cfg": params.get('cfgScale', ""), "method": params.get('samplingMethod', ""),
                    "size": f"{params.get('width')}x{params.get('height')}", "id": node.get('id'),
