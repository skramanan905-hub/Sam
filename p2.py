...
H_SEARCH = "4d76952c681f7d078707ddeec310f6475ab059e50546248120617abfb4031e9"
...
# --- REFERENCE IMAGE UPLOAD (EXACT 3-STEP BOT LOGIC) ---
...
def upload_logic():
    ...
    try:
        ...
        # Step 3: Finalize with ExternalID
...
def generate_logic():
    ...
    payload = {
        ...
        "variables": {
            "parameters": {
                "prompts": prompt + triggers, "modelId": "1861558740588989558", 
                "width": 512, "height": 1024, "batchSize": batch, 
                "lora": weights, "loraParameters": params, 
                "mediaId": mid, "strength": strength, 
...
# --- SEARCH, CREDITS, HISTORY (EXACT BOT HASHES) ---
...
def history():
    t = request.json.get("token")
    p = {"operationName":"listMyTasks","variables":json.dumps({"last":30,"parameterFields":["extra","prompts"]}),"extensions":json.dumps({"persistedQuery":{"version":1,"sha256Hash":H_LIST}})}
    r = requests.get(API_URL, params=p, headers=get_h(t)).json()
...
def claim():
    ...
    for platform in ["tiktok", "youtube", "instagram", "twitter", "discord"]:
        requests.post(API_URL, json={"operationName":"followSocialMedia","variables":{"platform":platform},"extensions":{"persistedQuery":{"version":1,"sha256Hash":H_REW}}}, headers=h)
