...
def clean_txt(text):
    if not text or str(text).lower() == "null": return "None"
...
def search():
    ...
    p = {"operationName": "listGenerationModels", "variables": json.dumps(v), "extensions": json.dumps({"persistedQuery": {"version": 1, "sha256Hash": H_SEARCH}})}
    ...
    return jsonify({"results": items, "cursor": page.get('endCursor') if page.get('hasNextPage') else None})

@app.route('/api/lora_meta', methods=['POST'])
def lora_meta():
    d = request.json
    p = {"operationName": "getGenerationModel", "variables": json.dumps({"id": d.get("id")}), "extensions": json.dumps({"persistedQuery": {"version": 1, "sha256Hash": H_META}})}
    r = requests.get(API_URL, params=p, headers=get_h(d.get("token")))
    res = r.json()
...
# ================= GENERATION & DETAILED HISTORY (FIXED MAPPING) =================
...
def generate():
    ...
    payload = {
        ...
        "variables": {
            "parameters": {
                "prompts": d.get("prompt") + ", " + all_t, "negativePrompts": d.get("neg", ""), "modelId": d.get("modelId"), 
                "width": int(d.get("w", 832)), "height": int(d.get("h", 1248)), "batchSize": int(d.get("batch", 1)), 
                "seed": "", "priority": 1000, "lora": l_w, "loraParameters": l_p, "mediaId": d.get("mediaId"), 
                "strength": float(d.get("strength", 0.55)), "samplingSteps": int(d.get("steps", 28)), "samplingMethod": "Euler a", 
                "cfgScale": float(d.get("cfg", 12.7)), "promptHelper": {"withStage": True, "userWantToEnable": True, "enable": True}
            }, 
...
def check_task():
    ...
    return jsonify({"status": status, "raw": sr, "images": imgs})

@app.route('/api/tasks', methods=['POST'])
def tasks():
    d = request.json
    # PARAMETER FIELDS ADDED TO PREVENT NULL/NA
    p_fields = ["prompts", "negativePrompts", "samplingSteps", "cfgScale", "width", "height", "loraParameters", "mediaId", "extra"]
    ...
    for edge in reversed(edges):
        ...
        if node.get('status') == "completed" and node.get('media'):
            ...
            orig_p = extra.get('naturalPrompts', "N/A")
            if isinstance(orig_p, list) and len(orig_p) > 0: orig_p = orig_p[0]

            task_data.append({
                "url": node['media']['urls'][0]['url'], "id": node['id'],
                "p_orig": clean_txt(orig_p), "p_final": clean_txt(p_node.get('prompts')), "neg": clean_txt(p_node.get('negativePrompts')),
                "size": f"{p_node.get('width',0)}x{p_node.get('height',0)}", "steps": p_node.get('samplingSteps', "N/A"), "cfg": p_node.get('cfgScale', "N/A"),
    ...
    return jsonify({"status": "success", "tasks": task_data, "cursor": resp_json['data']['me']['tasks']['pageInfo']['startCursor']})

# ================= ALL ORIGINAL TOOLS (RESTORED) =================
...
def claim_old():
    ...
    for p in ["tiktok", "youtube", "instagram", "twitter", "discord"]:
        r = requests.post(API_URL, json={"operationName":"followSocialMedia","variables":{"platform":p},"query":"mutation followSocialMedia($platform: String!) { followSocialMedia(platform: $platform) { success __typename } }"}, headers=h)
...
def enable_18():
    r = requests.post(API_URL, json={"operationName":"setPreferences","variables":{"value":{"ageVerificationStatus":"OVER18"}},"extensions":{"persistedQuery":{"version":1,"sha256Hash":H_18PLUS}}}, headers=get_h(request.json.get("token")))
    return jsonify({"status": "success", "raw": r.text})
