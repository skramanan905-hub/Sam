...
def get_h(t): 
    return {
        ...
        "User-Agent": "Mozilla/5.0 (Linux; Android 15; I2301) AppleWebKit/537.36"
    }
...
# --- GENERATION (FIXED MEDIAID SYNC) ---
@app.route('/api/generate', methods=['POST'])
def run_generation():
    d = request.json
    token, prompt, lora_ids = d.get("token"), d.get("prompt"), d.get("loras", [])
    
    # Corrected 'mediaId' to match the variable in your working script
    batch, mediaId = int(d.get("batch", 1)), d.get("mediaId")
    strength, lora_str = float(d.get("strength", 0.55)), float(d.get("lora_str", 0.7))
    width, height = int(d.get("w", 832)), int(d.get("h", 1248))
    steps, cfg = int(d.get("steps", 28)), float(d.get("cfg", 12.7))
    neg, user_triggers = d.get("neg", ""), d.get("triggers", "").strip()

    l_w, l_p, fetched_triggers = {}, [], ""
    for lid in lora_ids:
        meta = fetch_lora_meta(token, lid)
        if meta:
            l_w[meta['v_id']] = lora_str
            fetched_triggers += f"{meta['trigger']}, "
            active_trigger = user_triggers if user_triggers else meta['trigger']
            l_p.append({"versionId": meta['v_id'], "weight": lora_str, "triggerWords": active_trigger, "positionInfo": {"startIndex": 0, "endIndex": 0}})

    final_triggers = user_triggers if user_triggers else fetched_triggers
    full_p = prompt + ", " + final_triggers
    
    payload = {
        "operationName": "createGenerationTask",
        "variables": {
            "parameters": {
                "prompts": full_p, "negativePrompts": neg,
                "modelId": "1861558740588989558", "width": width, "height": height, "batchSize": batch,
                "lora": l_w, "loraParameters": l_p, 
                "mediaId": mediaId, "strength": strength, # Fixed mediaId inclusion
    ...
    try:
        ...
        for edge in reversed(edges):
            ...
            if node.get('status') == "completed":
                img_url = node.get('media', {}).get('urls', [{}])[0].get('url')
                if img_url:
                    task_data.append({"url": img_url, "prompt": clean_txt(node.get('parameters', {}).get('prompts', ""))})
        page_info = r['data']['me']['tasks']['pageInfo']
        return jsonify({"status": "success", "tasks": task_data, "cursor": page_info['startCursor'] if page_info['hasPreviousPage'] else None})
    except: return jsonify({"status": "error"})

# --- SEARCH, CREDITS, CLAIM (UNCHANGED) ---
