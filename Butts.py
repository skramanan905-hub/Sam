...
# UPDATED HASHES FROM YOUR AUG 27 LOGS
...
H_MODEL_SEARCH = "b7a2d663bc0381dd6eb26f8c68f702cb928bea720982f6f5553ea1629a8e871d" # listGenerationModels
...
# ================= SEARCH MODELS & LORAS (NEW GET LOGIC) =================
@app.route('/api/search_models', methods=['POST'])
def search_models():
    d = request.json
    v = {"first": 36, "types": ["ANY_MODEL"], "feed": d.get("feed", "meilisearch"), "keyword": d.get("keyword"), "after": d.get("cursor")}
    params = {"operationName": "listGenerationModels", "variables": json.dumps(v), "extensions": json.dumps({"clientLibrary": {"name": "@apollo/client", "version": "4.1.4"}, "persistedQuery": {"version": 1, "sha256Hash": H_MODEL_SEARCH}})}
    r = requests.get(API_URL, params=params, headers=get_h(d.get("token")))
    res = r.json()
    items = []
    for e in res['data']['generationModels']['edges']:
        n = e['node']
        items.append({"name": n['title'], "id": n['id'], "thumb": next((u['url'] for u in n['media']['urls'] if u['variant'] == "STILL_THUMBNAIL"), n['media']['urls'][0]['url']), "usage": fmt_num(n.get('refCount')), "likes": fmt_num(n.get('likedCount'))})
...
def search_loras():
    d = request.json
    v = {"first": 36, "types": ["ANY_LORA"], "loraBaseModelTypes": ["SDXL_MODEL"], "feed": d.get("feed", "meilisearch"), "keyword": d.get("keyword"), "after": d.get("cursor")}
    params = {"operationName": "listGenerationModels", "variables": json.dumps(v), "extensions": json.dumps({"clientLibrary": {"name": "@apollo/client", "version": "4.1.4"}, "persistedQuery": {"version": 1, "sha256Hash": H_MODEL_SEARCH}})}
    r = requests.get(API_URL, params=params, headers=get_h(d.get("token")))
    res = r.json()
    items = []
...
# ================= GENERATE (PRESERVING YOUR PARAMETERS) =================
...
def check_task():
    ...
    r_poll = requests.get(API_URL, params=params, headers=get_h(d.get("token")))
    sr = r_poll.json()
...
# ================= TASKS (PRESERVING YOUR HISTORY PARSING) =================
@app.route('/api/tasks', methods=['POST'])
def tasks():
    d = request.json
    vars = {"last": 30}
    if d.get("cursor"): vars["before"] = d.get("cursor")
    params = {"operationName": "listMyTasksTyped", "variables": json.dumps(vars), "extensions": json.dumps({"persistedQuery": {"version": 1, "sha256Hash": H_LIST}})}
    r = requests.get(API_URL, params=params, headers=get_h(d.get("token")))
    resp_json = r.json()
    task_data = []
    ...
    for edge in reversed(edges):
        ...
        if node['status'] == "completed" and node.get('media'):
            # Preservation of your complex prompt/lora parsing
    ...
    pInfo = resp_json['data']['me']['tasks']['pageInfo']
    return jsonify({"status": "success", "tasks": task_data, "cursor": pInfo.get('startCursor') if pInfo.get('hasPreviousPage') else None, "refreshed_token": check_refresh(r)})

# ================= ALL CLAIM OPTIONS (PRESERVED) =================
