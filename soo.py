...
H_COST   = "6201a4f00662d5158652433990f15f02c6114817a3a5f8674d82343719a9364a" # New: computeTaskCost
...
# New: Format counts like 166.15m
def format_num(n):
...
def compute_cost():
    d = request.json
    token = d.get("token")
    # Same parameters as generate but for cost preview
    l_w = {conf['v_id']: float(conf['weight']) for conf in d.get("lora_configs", [])}
    payload = {"operationName":"computeTaskCost","variables":{"parameters":{"prompts":"","modelId":d.get("modelId"),"width":int(d.get("w",832)),"height":int(d.get("h",1248)),"batchSize":int(d.get("batch",1)),"lora":l_w,"samplingSteps":int(d.get("steps",28)),"cfgScale":float(d.get("cfg",12.7))}},"extensions":{"persistedQuery":{"version":1,"sha256Hash":H_COST}}}
    try:
        r = requests.post(API_URL, json=payload, headers=get_h(token))
        cost = r.json()['data']['computeTaskCost']['baseCost']
...
def search_models():
    d = request.json
    # Filter logic: default to TRENDING if not provided
    sort_type = d.get("sort", "TRENDING") 
    v = {"keyword": d.get("keyword"), "feed": "meilisearch", "types": ["ANY_MODEL"], "first": 30, "after": d.get("cursor"), "sort": sort_type}
    ...
    for e in res['data']['generationModels']['edges']:
        ...
        items.append({
            "name": node['title'],
            "id": node['latestAvailableVersion']['id'] if node.get('latestAvailableVersion') else node['id'],
            "thumb": next((u['url'] for u in node['media']['urls'] if u['variant'] == "STILL_THUMBNAIL"), ""),
            "usage": format_num(node['statistics']['usageCount']), # Real Stats
            "type": node.get('modelType', '').replace('SDXL_1_0', 'XL').replace('PIXAI_DIT_V1', 'DiT.1') # Real Detail
        })
...
def search_loras():
    d = request.json
    sort_type = d.get("sort", "TRENDING")
    v = {"keyword": d.get("keyword"), "feed": "meilisearch", "types": ["ANY_LORA"], "first": 30, "after": d.get("cursor"), "sort": sort_type}
