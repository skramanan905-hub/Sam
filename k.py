...
def tasks():
    d = request.json
    p = {"operationName": "listMyTasks", "variables": json.dumps({"last": 30, "before": d.get("cursor"), "parameterFields": ["extra", "prompts", "negativePrompts", "samplingSteps", "samplingMethod", "cfgScale", "width", "height", "modelId", "mediaId", "strength", "loraParameters"]}), "extensions": json.dumps({"persistedQuery": {"version": 1, "sha256Hash": H_LIST}})}
    try:
        ...
        for edge in reversed(edges):
            ...
            if node['status'] == "completed":
                img = node['media']['urls'][0]['url']
                params = node['parameters']
                extra = params.get('extra', {})
                
                natural = extra.get('naturalPrompts', [])
                if isinstance(natural, list) and len(natural) > 0: orig = natural[0]
                elif extra.get('naturalPrompt'): orig = extra['naturalPrompt']
                else: orig = params.get('prompts', 'N/A')

                ref_id = params.get('mediaId')
                ref_thumb = f"https://api.pixai.art/v1/media/{ref_id}/thumbnail" if ref_id else None

                task_data.append({
                    "url": img, "p_orig": clean_txt(orig), "p_final": clean_txt(params.get('prompts')),
                    "neg": clean_txt(params.get('negativePrompts', "")), "id": node['id'],
                    "time": format_pixai_time(node.get('createdAt')), "size": f"{params.get('width')}x{params.get('height')}",
                    "steps": params.get('samplingSteps'), "cfg": params.get('cfgScale'),
                    "method": params.get('samplingMethod'), "ref_url": ref_thumb,
                    "model": params.get('modelId', "Unknown"),
