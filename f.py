...
# --- 1. FULL HISTORY LOGIC (NEWEST FIRST + DEEP METADATA) ---
...
def tasks():
    ...
    try:
        ...
        task_data = []
        for edge in reversed(edges): # Logic: Reverse to put newest at top
            node = edge['node']
            if node['status'] == "completed":
                img = node['media']['urls'][0]['url']
                p_node = node['parameters']
                # Logic: Fetch original prompt from extra naturalPrompts array
                original = p_node.get('extra', {}).get('naturalPrompts', [""])[0]
                final = p_node.get('prompts', "")
                task_data.append({
                    "url": img, "p_orig": clean_txt(original if original else final), "p_final": clean_txt(final),
                    "neg": clean_txt(p_node.get('negativePrompts', "")), "steps": p_node.get('samplingSteps'),
                    "cfg": p_node.get('cfgScale'), "method": p_node.get('samplingMethod'),
                    "size": f"{p_node.get('width')}x{p_node.get('height')}", "id": node['id'], "model": p_node.get('modelId')
                })
...
# --- 2. MODEL SEARCH LOGIC (VERSION ID SAFETY CHECK) ---
...
def search_models():
    ...
    try:
        ...
        for e in res['data']['generationModels']['edges']:
            n = e['node']
            # Safety Logic: Extract Engine ID (Version) or fallback to Model ID
            ...
            items.append({"name": n['title'], "id": engine_id, "thumb": thumb})
...
# --- 3. GENERATION (CORE LOGIC + INDEPENDENT LORAS) ---
