...
# --- MANDATORY FOR RENDER WEB SERVICE ---
# This part stops the "No open ports detected" error
app = Flask('')
@app.route('/')
...
def get_headers(token):
    return {"Authorization": f"Bearer {token.strip()}", "Content-Type": "application/json", "x-browser-id": BROWSER_ID, "User-Agent": UA}

def fetch_lora_info(token, lora_id):
    params = {"operationName": "getGenerationModel", "variables": json.dumps({"id": lora_id}), "extensions": json.dumps({"persistedQuery": {"version": 1, "sha256Hash": HASH_LORA}})}
    try:
...
def welcome(message):
    ...
    markup.add("🔑 Login", "💰 Balance", "🎨 Start Generating", "🎁 Claim Daily")
    bot.send_message(uid, "🚀 **PixAI Pro Console**\nReady for generation on Render.", reply_markup=markup, parse_mode="Markdown")
...
def get_prompt(message):
    ...
    msg = bot.send_message(message.chat.id, "🧬 **STEP 2:** Enter LoRA IDs (space separated) or type 'none':")
...
def get_batch(message):
    ...
    # ADDED BATCH SIZE BUTTONS HERE
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True, one_time_keyboard=True)
    markup.add("1", "2", "3", "4")
    msg = bot.send_message(message.chat.id, "🔢 **STEP 3:** Choose Batch Size (1-4):", reply_markup=markup)
...
def start_pixai_process(message):
    ...
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add("🔑 Login", "💰 Check Credits", "🎨 Start Generating", "🎁 Claim Daily")
    ...
    try:
        ...
        while True:
            time.sleep(10)
            poll_params = {"operationName": "getTaskById", "variables": json.dumps({"id": task_id}), "extensions": json.dumps({"persistedQuery": {"version": 1, "sha256Hash": HASH_POLL}})}
...
if __name__ == "__main__":
    # This thread runs the small server Render requires
