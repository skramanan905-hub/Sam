    ...
    try:
        ...
        for edge in reversed(edges):
            ...
            if node.get('status') == "completed":
                ...
                original = natural[0] if (isinstance(natural, list) and len(natural)>0) else params.get('prompts', "")
                
                ref_id = params.get('mediaId')
                ref_thumb = f"https://api.pixai.art/v1/media/{ref_id}/thumbnail" if ref_id else None

                task_data.append({
                    "url": img, "p_orig": clean_txt(original), "p_final": clean_txt(params.get('prompts')),
                    "neg": clean_txt(params.get('negativePrompts', "")), "steps": params.get('samplingSteps', ""),
                    "cfg": params.get('cfgScale', ""), "method": params.get('samplingMethod', ""),
                    "size": f"{params.get('width')}x{params.get('height')}", "id": node.get('id'),
                    "model": params.get('modelId', "Unknown"), "ref_url": ref_thumb,
...
def search_models():
    ...
    try:
        ...
        for e in res['data']['generationModels']['edges']:
            n = e['node']
            version_id = n.get('latestAvailableVersion', {}).get('id', n['id'])
            thumb = next((u['url'] for u in n['media']['urls'] if u['variant'] == "STILL_THUMBNAIL"), "")
            items.append({"name": n['title'], "id": version_id, "thumb": thumb})
