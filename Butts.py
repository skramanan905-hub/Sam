...
def search_unified():
    ...
    # NEW GET REQUEST LOGIC FROM listm.txt
    params = {
...
# ================= GENERATION & TASKS (ORIGINAL LOGIC) =================
...
def tasks():
    ...
    for edge in reversed(edges):
        ...
        if node['status'] == "completed":
            ...
            natural_data = extra.get('naturalPrompts', "N/A")
            task_data.append({
                "url": node['media']['urls'][0]['url'] if node.get('media') else "", 
                "p_orig": clean_txt(natural_data), 
