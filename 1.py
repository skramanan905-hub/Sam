...
H_MARK   = "98ec6dc4d4e288b92fed763241f14f65d7dace28de068e4180a90c1248cacdf4" # Bookmark Hash
...
def get_h(t): 
    return {
        ...
        "User-Agent": "Mozilla/5.0 (Linux; Android 15; I2301) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/146.0.7680.120 Mobile Safari/537.36"
    }
...
def lora_meta():
    ...
    try:
        ...
        n = res['data']['generationModel']
        v = n['latestAvailableVersion']
        thumb = next((u['url'] for u in n['media']['urls'] if u['variant'] == "STILL_THUMBNAIL"), "")
        return jsonify({"v_id": v['id'], "trigger": v['extra'].get('triggerWords', ""), "name": n['title'], "thumb": thumb})
    ...
    payload = {
        ...
        "variables": {
            "parameters": {
                ...
                "modelId": "1861558740588989558", "width": width, "height": height, "batchSize": batch,
    ...
    try:
        ...
        for edge in reversed(edges): # Newest First
            ...
            if n['status'] == "completed":
                ...
                fmt = "%Y-%m-%dT%H:%M:%S.%fZ"
                try:
                    c = datetime.strptime(n['createdAt'], fmt)
                    s = datetime.strptime(n['startedAt'], fmt)
                    e = datetime.strptime(n['endedAt'], fmt)
                    wait = int((s - c).total_seconds())
                    gen = int((e - s).total_seconds())
                except: wait, gen = 0, 0
                ...
                task_data.append({
                    ...
                    "wait": wait, "gen": gen, "model": n['parameters'].get('modelId', "Haruka v2"),
...
def search():
    d = request.json
    t_type = d.get("type", "ANY_LORA") # ANY_LORA or ANY_MODEL
    v = {"keyword": d.get("keyword"), "feed": "meilisearch", "types": [t_type], "first": 30, "after": d.get("cursor")}
    ...
    items = []
    for e in r['data']['generationModels']['edges']:
        n = e['node']
        items.append({"name": n['title'], "id": n['id'], "thumb": next((u['url'] for u in n['media']['urls'] if u['variant'] == "STILL_THUMBNAIL"), "")})
