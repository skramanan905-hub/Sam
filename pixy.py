...
def welcome(message):
    uid = message.chat.id
    user_data[uid] = {'token': None, 'model': '1861558740588989558', 'batch': 1}
    markup = types.ReplyKeyboardMarkup(resize_keyboard=True)
    markup.add("🔑 Login", "💰 Check Credits")
    markup.add("🎨 Start Generating", "🎁 Claim Daily")
    bot.send_message(uid, "🚀 **PixAI Pro Console Bot v3.1**\n\nLogin with your token to start.", reply_markup=markup, parse_mode="Markdown")
...
# --- GENERATION FLOW (NOW WITH BATCH OPTION) ---
...
def get_loras(message):
    user_data[message.chat.id]['lora_raw'] = message.text
    msg = bot.send_message(message.chat.id, "🔢 **STEP 3:** Enter Batch Size (1, 2, 3, or 4):")
...
def start_pixai_process(message):
    ...
    try:
        ...
        if not (1 <= batch <= 4): raise ValueError
    except:
        return bot.send_message(uid, "❌ Invalid batch size (1-4). Start generating again.")

    lora_input = [] if u['lora_raw'].lower() == 'none' else u['lora_raw'].split()
    status_msg = bot.send_message(uid, "🛰 **Accessing PixAI Cloud...**")
    
    l_params, l_weights, triggers = [], {}, ""
    for lid in lora_input:
    ...
    try:
        ...
        if "errors" in res:
            return bot.edit_message_text(f"❌ **Error:** {res['errors'][0]['message']}", uid, status_msg.id)
        ...
        while True:
            ...
            if task['status'] == "completed":
                ...
                bot.edit_message_text("❌ PixAI server failed to render.", uid, status_msg.id)
                break
    except Exception as e:
        bot.send_message(uid, f"❌ Script Crash: {e}")

# --- START ---
if __name__ == "__main__":
    threading.Thread(target=run_web).start()
    print("--- PIXAI RENDER BOT IS ONLINE ---")
