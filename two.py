...
def run_generation():
    ...
    neg = d.get("neg", "nsfw, worst quality, low quality")
    ...
    for lid in lora_ids:
        ...
        if meta:
            ...
            all_t += f"{meta['trigger']}, "
            l_p.append({"versionId": meta['v_id'], "weight": lora_str, "triggerWords": meta['trigger'], "positionInfo": {"startIndex": 0, "endIndex": 0}})
    ...
    payload = {
        ...
        "variables": {
            "parameters": {
                ...
                "width": width, "height": height, "batchSize": batch,
            ...
            "extra": {"naturalPrompts": [prompt]} # Bypass Logic
        },
...
# --- 2. TASK LIST (REVERSED: NEWEST FIRST) ---
...
def get_web_tasks():
    ...
    try:
        ...
        for edge in reversed(edges): # Reversed for Newest First
            node = edge.get('node', {})
