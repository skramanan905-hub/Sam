...
# ================= MASTER CONFIGURATION (AUG 27 UPDATED) =================
    ...
    payload = {
        ...
        "variables": {
            "parameters": {
                ...
                "width": int(d.get("w")), "height": int(d.get("h")), 
                "batchSize": int(d.get("batch", 1)), 
                "seed": "", "priority": 1000, 
                "lora": l_w, "loraParameters": l_p, 
                "mediaId": d.get("mediaId"), # RESTORED
                "strength": float(d.get("strength", 0.55)), # RESTORED
    ...
    for edge in reversed(edges):
        ...
        if node['status'] == "completed" and node.get('media'):
            ...
            task_data.append({
                ...
                "id": node['id'], "time": format_pixai_time(node.get('createdAt')), 
                "size": f"{p_node.get('width')}x{p_node.get('height')}", 
                "steps": p_node.get('samplingSteps'), "cfg": p_node.get('cfgScale'), 
