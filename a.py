        ...
        try:
            ...
            for _ in range(35):
                ...
                if sr['data']['task']['status'] == "completed":
                    yield "RESULT:" + json.dumps({"status": "success", "images": [i['url'] for i in sr['data']['task']['media']['urls'] if i['variant'] == "PUBLIC"]}) + "\n"
                    return
            yield "RESULT:{\"status\":\"error\"}\n"
        except: yield "RESULT:{\"status\":\"error\"}\n"
    return Response(stream_with_context(run_stream()), mimetype='text/plain')

@app.route('/api/tasks', methods=['POST'])
def tasks():
    d = request.json
    p = {"operationName": "listMyTasks", "variables": json.dumps({"last": 30, "before": d.get("cursor"), "parameterFields": ["extra", "prompts", "negativePrompts", "samplingSteps", "samplingMethod", "cfgScale", "width", "height", "modelId", "mediaId", "strength"]}), "extensions": json.dumps({"persistedQuery": {"version": 1, "sha256Hash": H_LIST}})}
    try:
        ...
        for edge in reversed(edges):
            ...
            if node.get('status') == "completed":
                ...
                # MATCHED TO YOUR HISTORY SNIPPET
...
def search_models():
    ...
    # GRABS VERSION ID INSTEAD OF MODEL ID
