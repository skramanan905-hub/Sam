...
def welcome(message):
    ...
    bot.send_message(uid, "✨ **PixAI Pro Console v5.1**\nReady for action.", reply_markup=markup, parse_mode="Markdown")
...
def check_credits(message):
    ...
    except: bot.send_message(uid, "❌ Error checking balance. Token may be expired.")
...
def start_pixai_process(message):
    ...
    try:
        ...
        # FIX: Check if 'data' exists before trying to use it
        if not res or 'data' not in res or res['data'] is None:
            err_msg = res.get('errors', [{'message': 'Unknown API Error'}])[0]['message']
            return bot.send_message(uid, f"❌ **PixAI API Error:** {err_msg}")
        ...
        while True:
            ...
            if 'data' not in s_res or s_res['data'] is None: continue
