...
def get_h(t): 
    return {
        ...
        "User-Agent": "Mozilla/5.0 (Linux; Android 15; I2301) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/145.0.0.0 Mobile Safari/537.36"
    }

def clean_txt(text):
    return re.sub(r'[_*`\[\]()~>#+\-={}|.!]', '', str(text)) if text else ""

# --- UPDATED: HISTORY WITH PRO DETAILS ---
@app.route('/api/tasks', methods=['POST'])
def get_pro_tasks():
    d = request.json
    t, cursor = d.get("token"), d.get("cursor")
    p = {"operationName": "listMyTasks", "variables": json.dumps({"last": 30, "before": cursor, "parameterFields": ["extra", "prompts"]}), "extensions": json.dumps({"persistedQuery": {"version": 1, "sha256Hash": H_LIST}})}
    try:
        r = requests.get(API_URL, params=p, headers=get_h(t), timeout=15).json()
        edges = r['data']['me']['tasks']['edges']
        task_data = []
        fmt = "%Y-%m-%dT%H:%M:%S.%fZ"
        for edge in reversed(edges): # Newest First
            n = edge['node']
            if n['status'] == "completed":
                # Time Calculations
                try:
                    c_at, s_at, e_at = datetime.strptime(n['createdAt'], fmt), datetime.strptime(n['startedAt'], fmt), datetime.strptime(n['endedAt'], fmt)
                    wait, gen = int((s_at - c_at).total_seconds()), int((e_at - s_at).total_seconds())
                except: wait, gen = 0, 0
                
                task_data.append({
                    "id": n['id'], "url": n['media']['urls'][0]['url'], "prompt": clean_txt(n['parameters']['prompts']),
                    "neg": n['parameters'].get('negativePrompts', ""), "size": f"{n['parameters']['width']}x{n['parameters']['height']}",
                    "wait": f"{wait}s", "gen": f"{gen}s", "model": n['parameters'].get('modelId', "Haruka v2"),
                    "steps": n['parameters'].get('samplingSteps', 28), "cfg": n['parameters'].get('cfgScale', 5),
                    "sub": n['createdAt'], "start": n['startedAt']
                })
...
def handle_bookmark():
    d = request.json
    payload = {"operationName":"markGenerationModel","variables":{"id":d.get("id"),"target":d.get("target"),"type":"BOOKMARK"},"extensions":{"persistedQuery":{"version":1,"sha256Hash":H_MARK}}}
    requests.post(API_URL, json=payload, headers=get_h(d.get("token")))
...
def delete_task():
    d = request.json
    payload = {"operationName":"deleteTask","variables":{"id":d.get("id")},"extensions":{"persistedQuery":{"version":1,"sha256Hash":"48356f1577793d567c29379858369ce321a6e78336558ec50f0c23178401496e"}}}
    requests.post(API_URL, json=payload, headers=get_h(d.get("token")))
    return jsonify({"status": "success"})

# --- SEARCH (MODEL/LORA TOGGLE) ---
...
# --- ALL EXISTING GENERATION & UPLOAD LOGIC (UNCHANGED) ---
...
def lora_meta():
    ...
    try:
        ...
        return jsonify({"v_id": v['id'], "trigger": v['extra'].get('triggerWords', ""), "name": res['data']['generationModel']['title'], "thumb": next((u['url'] for u in res['data']['generationModel']['media']['urls'] if u['variant'] == "STILL_THUMBNAIL"), "")})
...
def run_generation():
    ...
    payload = {"operationName": "createGenerationTask", "variables": {"parameters": {"prompts": prompt + ", " + all_t, "negativePrompts": neg, "modelId": "1861558740588989558", "width": w, "height": h, "batchSize": batch, "lora": l_w, "loraParameters": l_p, "mediaId": mediaId, "strength": strength, "samplingSteps": steps, "samplingMethod": "Euler a", "cfgScale": cfg, "promptHelper": {"withStage": True, "userWantToEnable": True, "enable": True}}, "extra": {"naturalPrompts": [prompt]}}, "extensions": {"persistedQuery": {"version": 1, "sha256Hash": H_GEN}}}
    try:
