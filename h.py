...
# --- FOCUS: DEEP HISTORY LOGIC (SYNCED WITH YOUR 2ND CODE) ---
@app.route('/api/tasks', methods=['POST'])
def tasks():
    d = request.json
    t, cursor = d.get("token"), d.get("cursor")
    # Parameter fields exactly from your 2nd logic code
    ...
    try:
        ...
        task_data = []
        for edge in reversed(edges): # Reversed for Newest on Top
            node = edge['node']
            if node['status'] == "completed":
                params = node['parameters']
                extra = params.get('extra', {})
                
                # Original Prompt Extraction from 2nd code
                orig_prompt = "Not Found"
                natural_data = extra.get('naturalPrompts', [])
                if isinstance(natural_data, list) and len(natural_data) > 0:
                    orig_prompt = natural_data[0]
                elif extra.get('naturalPrompt'):
                    orig_prompt = extra['naturalPrompt']
                else:
                    orig_prompt = params.get('prompts', 'N/A')

                # LoRA Extraction logic
                loras_applied = [{"trigger": l.get('triggerWords'), "weight": l.get('weight')} for l in params.get('loraParameters', [])]

                # FIXED: Reference Image URL Logic as requested
                ref_id = params.get('mediaId')
                ref_thumb_url = f"https://api.pixai.art/v1/media/{ref_id}/thumbnail" if ref_id else None
                ...
                task_data.append({
                    ...
                    "id": node['id'],
                    "time_created": format_time(node.get('createdAt')),
                    "time_started": format_time(node.get('startedAt')),
                    "wait": node.get('waitedTime', '0'),
                    "elapsed": node.get('elapsedTime', '0'),
                    ...
                    "ref_url": ref_thumb_url, # NEW URL LOGIC
                    "loras": loras_applied
                })
...
def search_models():
    ...
    try:
        ...
        for e in res['data']['generationModels']['edges']:
            n = e['node']
            latest_v = n.get('latestAvailableVersion')
            engine_id = latest_v.get('id', n['id']) if (latest_v and isinstance(latest_v, dict)) else n['id']
            items.append({"name": n['title'], "id": engine_id, "thumb": next((u['url'] for u in n['media']['urls'] if u['variant'] == "STILL_THUMBNAIL"), "")})
...
def generate():
    ...
    lora_configs, helper = d.get("lora_configs", []), d.get("helper", True)
