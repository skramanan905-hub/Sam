...
# NEW SEARCH HASH FROM new search.py
H_NEW_SEARCH = "b7a2d663bc0381dd6eb26f8c68f702cb928bea720982f6f5553ea1629a8e871d"
...
# --- NEW SEARCH ENGINE (FIXED USING new search.py) ---
@app.route('/api/search', methods=['POST'])
def search():
    d = request.json
    v = {
        "first": 24, 
        "types": [d.get("type")], # ANY_MODEL or ANY_LORA
        "feed": d.get("feed", "meilisearch"), # trending, meilisearch, or top_liked
        "keyword": d.get("keyword", ""),
        "after": d.get("cursor")
    }
    # Optional filter for LoRAs
    if d.get("type") == "ANY_LORA":
        v["loraBaseModelTypes"] = ["SDXL_MODEL"]

    payload = {
        "operationName": "listGenerationModels",
        "variables": v,
        "extensions": {"persistedQuery": {"version": 1, "sha256Hash": H_NEW_SEARCH}}
    }
    ...
    for e in res['data']['generationModels']['edges']:
        n = e['node']
        # If searching models, return the Version ID as the ID
        m_id = n['latestAvailableVersion']['id'] if d.get("type") == "ANY_MODEL" and n.get('latestAvailableVersion') else n['id']
        items.append({
            "name": n['title'], "id": m_id, 
            "thumb": next((u['url'] for u in n['media']['urls'] if u['variant'] == "STILL_THUMBNAIL"), ""),
            "usage": fmt_num(n.get('refCount')), "likes": fmt_num(n.get('likedCount')), "type": fmt_type(n.get('type'))
        })
...
def generate():
    d = request.json
    token, lora_configs = d.get("token"), d.get("lora_configs", [])
    l_w, l_p, all_t = {}, [], ""
    ...
    while True:
        ...
        if sr['data']['task']['status'] == "completed":
            return jsonify({"status": "success", "images": [i['url'] for i in sr['data']['task']['media']['urls'] if i['variant'] == "PUBLIC"], "refreshed_token": check_refresh(r_poll), "req_log": payload, "res_log": sr})
        if sr['data']['task']['status'] == "failed": return jsonify({"status": "error", "res_log": sr})
