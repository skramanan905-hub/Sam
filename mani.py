...
def generate():
    ...
    modelId = d.get("modelId") # Selected model from PHP
    ...
    try:
        ...
        for edge in reversed(edges):
            ...
            if node.get('status') == "completed":
                ...
                # FIX: PixAI stores clean prompt in naturalPrompts and full string in prompts
                original = node.get('extra', {}).get('naturalPrompts', [""])[0]
                final = node.get('parameters', {}).get('prompts', "")
                if not original: original = final # Fallback
                if img: task_data.append({"url": img, "prompt_original": clean_txt(original), "prompt_final": clean_txt(final)})
...
# Credits, Claim, Search, Upload remain same
